let roleHarvester = {
    run: function (creep) {
        ...
        let resources = creep.room.find(FIND_DROPPED_RESOURCES, {filter: (resources) => {return (resources.amount > 350)}});
        ...
        if (creep.store.getFreeCapacity() == creep.store.getCapacity() || (spawn == '' && tower == '' && creep.store.getFreeCapacity() > 0)) {
            ...
            if (spawn != '' && creep.store.getUsedCapacity(RESOURCE_ENERGY) > 300) {
                const clost = creep.pos.findClosestByRange(spawn);
                if (creep.transfer(clost, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                    creep.moveTo(clost);
                }
            }
            else if (extension != '' && creep.store.getUsedCapacity(RESOURCE_ENERGY) > 50) {
